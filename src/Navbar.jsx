...
const Navbar = () => {
  ...
  return (
    <section className="fixed top-0 left-0 z-50 w-full">
      {/* Navbar */}
      <div className="relative w-full bg-white/4">
        <div className="mx-auto flex max-w-360 items-center justify-between gap-6 px-5 py-2 sm:px-8 lg:gap-14 lg:px-10">
          {/* Logo */}
        ...
        {/* Mobile Navigation */}
        {isMenuOpen && (
          <ul className="absolute left-0 top-full flex w-full flex-col items-start gap-5 bg-black/90 px-5 py-6 text-sm tracking-wide backdrop-blur-sm sm:px-8 lg:hidden">
            {navOptions.map((option) => (
              <li key={option.id} className="w-full">
                <a
                  href={`#${option.id}`}
                  onClick={handleNavClick(option.id)}
                  className="block w-full cursor-pointer text-gray-300 transition-colors hover:text-white"
                >
                  {option.name}
                </a>
              </li>
